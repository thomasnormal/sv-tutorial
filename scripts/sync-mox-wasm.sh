...
# Browser compatibility shim for tools linked with NODERAWFS (mox-sim,
# mox-verilog, mox-bmc). These builds embed Node-only path/fs glue that throws
# outside Node; in the browser worker the tool then fails silently (exit 1, no
# output). Rewriting these sections off NODERAWFS lets the worker back them with
...
function patchFile(simPath) {
  ...
  const replaceExact = (regex, replacement, label) => {
    ...
    if (source.includes(replacement)) {
      console.log(`Skipped patch (${label}): already applied in ${simPath}`);
      return;
    }
